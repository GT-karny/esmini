...
| Gate | Command | Scope |
| :--- | :--- | :--- |
| **Unit** | `scripts/run_gt_tests.ps1` | ctest `test_ScenarioReaderParsing` = 25 unit sources を束ねた傘バイナリ（RealDriver utils / LonProfilePlanner / VD policies / ODR road層13本 / 他）。Green; runs in CI. **名前に反し `GT_ScenarioReader` のパースは非カバー**（capability_model.md §2 参照）。 |
| **Pre-merge regression** | `scripts/run_regression_gate.ps1` | Step 1 = unit gate (hard). Step 1.5 = ODR conformance quick (hard, `-SkipOdr` to skip). Step 2 = VirtualDriver behavioral batch (`gt_sim_test.py batch resources/xosc/verification/phase3_batch.yaml`, in-process via `GT_esminiLib.dll`, venv `DriverScript/.venv`), then **per-scenario / per-matcher** compare against the committed baseline `GT_esmini/test/regression_baseline/phase3_expected.yaml` via `scripts/check_phase3_regression.py` (the batch's own coarse `overall=fail` is intentionally NOT the gate — the per-scenario deviation vs baseline is). WARN by default; `-FailOnBehavioral` gates on baseline deviation. Requires a completed Release build. Additional Phase-3 corridor/junction manifests exist for manual runs: `phase3d_batch.yaml`, `phase3d_crosswalk_batch.yaml`, `phase3d_crosswalk_crosspath_batch.yaml`, `phase3e_batch.yaml`. |
...
- **CI** (`.github/workflows/ci.yml`): the `test` job runs the GT unit ctest step after upstream `run_tests.sh` (audit TST-1 closed), then the fork-sync warning step (above). A separate Windows job runs the VirtualDriver behavioral regression (`gt_sim_test.py batch phase3_batch.yaml` → `check_phase3_regression.py` vs committed baseline, non-blocking; uploads `regression_report.md`).
