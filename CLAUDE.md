...
| **Pre-merge regression** | `scripts/run_regression_gate.ps1` | Step 1 = unit gate (hard). Step 1.5 = ODR conformance quick (hard, `-SkipOdr` to skip). Step 1.6 = OSI logical lane probes (hard, `-SkipOsiProbes` to skip): 5 in-process probes (~13 s) covering junction connectivity, boundary 5cm/2cm deviation, lane assignment, `HostVehicleData.route` and the per-frame payload invariant — added because Step 2 exercises only ONE road asset, so the junction/boundary claims are unreachable from it and would rot as manual probes (`gate:osi-logical-lane-probes`). Step 2 = VirtualDriver behavioral batch (`gt_sim_test.py batch resources/xosc/verification/car_following_traffic_control_batch.yaml`, in-process via `GT_esminiLib.dll`, venv `DriverScript/.venv`), then **per-scenario / per-matcher** compare against the committed baseline `GT_esmini/test/regression_baseline/car_following_traffic_control_expected.yaml` via `scripts/check_regression_baseline.py` (the batch's own coarse `overall=fail` is intentionally NOT the gate — the per-scenario deviation vs baseline is). WARN by default; `-FailOnBehavioral` gates on baseline deviation. Requires a completed Release build. Additional Phase-3 corridor/junction manifests exist for manual runs: `junction_conflict_batch.yaml`, `crosswalk_pedestrian_batch.yaml`, `crosswalk_crosspath_batch.yaml`, `junction_priority_batch.yaml`. |
| **ManualDrive ADAS regression** | `scripts/run_regression_gate.ps1` Step 2.10 | 人間運転中に並行稼働する ADAS（AEB/FCW/ACC/LKA/LDW/MSL）の挙動回帰。`manualdrive_adas_batch.yaml` 30シナリオ vs committed baseline。既定WARN・`-SkipManualAdas`。詳細（covers / not_covers / 昇格手順）は `GT_esmini/docs/knowledge/gate_catalog.yaml` の `gate:manualdrive-adas-regression`。 |
