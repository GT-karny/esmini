# AEB c2c CCRs 自車20km/h vs 停止先行車

直線路で自車が20km/hから、同一車線上に停止した先行車へ接近する。
...
| 自車 | road 1 / lane -1 / s=30.0、初期速度20km/h（5.5556m/s）、VirtualDriverController（lateral/longitudinal）有効、ルートはs=30.0→s=480.0 |
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=75.0、速度0km/h（停止）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 11.6 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
自車速度20km/hでの closing_speed（=自車速度、先行車は停止のため）は5.5556 m/s、
`5.5556 × 6.0 = 33.33` m だが下限の40.0 mでフロアされ、gap=40.0 mとなる。
車両原点間距離はこれに車長5.0 mを加えた45.0 mで、自車s=30.0に対し先行車s=75.0（30.0+40.0+5.0）となる。

## 進行

1. t=0: 自車は20km/hで走行開始、VirtualDriverControllerがルート追従を開始する。
2. 先行車は速度0km/hのまま静止し続ける。
3. 自車が先行車に接近するにつれ、AEBポリシーの介入が期待される領域に入る。
4. t=11.6s超過でStopTriggerが成立し、シナリオが終了する。
