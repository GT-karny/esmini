# AEB c2c CCRs 自車30km/h vs 停止先行車

直線路で自車が30km/hから、同一車線上に停止した先行車へ接近する。
...
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=85.0、速度0km/h（停止）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 11.1 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
自車速度30km/hでの closing_speed（=自車速度、先行車は停止のため）は8.3333 m/s、
`8.3333 × 6.0 = 50.0` m が下限の40.0 mを上回るため、gap=50.0 mとなる（TTC0=6.0sちょうど）。
車両原点間距離はこれに車長5.0 mを加えた55.0 mで、自車s=30.0に対し先行車s=85.0（30.0+50.0+5.0）となる。
...
4. t=11.1s超過でStopTriggerが成立し、シナリオが終了する。
