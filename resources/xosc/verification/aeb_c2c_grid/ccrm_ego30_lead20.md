# AEB c2c CCRm 自車30km/h vs 先行車20km/h（定速走行）

直線路で自車が30km/hから、同一車線上を20km/h一定で走行する先行車へ接近する。
...
自車速度と先行車速度の差で決まる状況でのAEB挙動を、CCRm系統の低速端（自車30km/h）で確認する。
...
| 自車 | road 1 / lane -1 / s=30.0、初期速度30km/h（8.3333m/s）、VirtualDriverController（lateral/longitudinal）有効、ルートはs=30.0→s=480.0 |
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=75.0、速度20km/h（5.5556m/s、定速）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 19.5 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
closing_speed（=自車速度−先行車速度）は8.3333−5.5556=2.7778 m/s、
`2.7778 × 6.0 = 16.67` m だが下限の40.0 mでフロアされ、gap=40.0 mとなる。
車両原点間距離はこれに車長5.0 mを加えた45.0 mで、自車s=30.0に対し先行車s=75.0（30.0+40.0+5.0）となる。

## 進行

1. t=0: 自車は30km/hで走行開始、VirtualDriverControllerがルート追従を開始する。
2. 先行車は20km/hの定速走行を続ける。
3. 自車と先行車の速度差（closing speed）に応じて自車が接近する。
4. t=19.5s超過でStopTriggerが成立し、シナリオが終了する。
