# AEB c2c CCRm 自車70km/h vs 先行車20km/h（定速走行）

直線路で自車が70km/hから、同一車線上を20km/h一定で走行する先行車へ接近する。
AEB（自動緊急ブレーキ）の刺激探索セルの1つで、CCRm（Car-to-Car Rear moving）系統の最高速度側。
...
自車速度と先行車速度の差で決まる状況でのAEB挙動を、CCRm系統の高速端（自車70km/h）で確認する。
...
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=118.333、速度20km/h（5.5556m/s、定速）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 13.9 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
closing_speed（=自車速度−先行車速度）は19.4444−5.5556=13.8889 m/s、
`13.8889 × 6.0 = 83.333` m が下限の40.0 mを上回るため、gap=83.333 mとなる（TTC0=6.0sちょうど）。
車両原点間距離はこれに車長5.0 mを加えた88.333 mで、自車s=30.0に対し先行車s=118.333（30.0+83.333+5.0）となる。
