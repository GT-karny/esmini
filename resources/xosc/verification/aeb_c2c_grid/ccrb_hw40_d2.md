# AEB c2c CCRb 車間40m・先行車減速2m/s^2
...
車間の広い側（40m）と減速の弱い側（2m/s^2）の組み合わせで確認する。
...
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=75.0、初期速度50km/h（13.8889m/s）、コントローラなし。t=2.0s以降、減速度2.0m/s^2（linear rate）で速度0まで減速するManeuver（優先度overwrite）を実行 |
