# AEB c2c CCRm 自車40km/h vs 先行車20km/h（定速走行）

直線路で自車が40km/hから、同一車線上を20km/h一定で走行する先行車へ接近する。
...
| 走行時間 | StopTrigger: シミュレーション時間 13.0 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
closing_speed（=自車速度−先行車速度）は11.1111−5.5556=5.5556 m/s、
...
4. t=13.0s超過でStopTriggerが成立し、シナリオが終了する。
