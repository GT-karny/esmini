# AEB c2c CCRm 自車50km/h vs 先行車20km/h（定速走行）

直線路で自車が50km/hから、同一車線上を20km/h一定で走行する先行車へ接近する。
...
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=85.0、速度20km/h（5.5556m/s、定速）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 12.5 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
closing_speed（=自車速度−先行車速度）は13.8889−5.5556=8.3333 m/s、
...
1. t=0: 自車は50km/hで走行開始、VirtualDriverControllerがルート追従を開始する。
2. 先行車は20km/hの定速走行を続ける。
3. 自車と先行車の速度差（closing speed）に応じて自車が接近する。
4. t=12.5s超過でStopTriggerが成立し、シナリオが終了する。
