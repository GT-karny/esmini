# AEB c2c CCRs 自車50km/h vs 停止先行車

直線路で自車が50km/hから、同一車線上に停止した先行車へ接近する。
...
| 他エンティティ | 先行車（Lead）: road 1 / lane -1 / s=118.333、速度0km/h（停止）、コントローラなし |
| 走行時間 | StopTrigger: シミュレーション時間 12.5 s 超過 |

初期車間（バンパー間）は `gap = max(40.0, closing_speed × 6.0)` で決まる。
自車速度50km/hでの closing_speed（=自車速度、先行車は停止のため）は13.8889 m/s、
