...
if [[ "$OSTYPE" =~ ^(msys|cygwin|linux-gnu) ]]; then
    ...
    # Still skipped here (out of this port's scope):
    #   - OSITunnelTestFixture.TestOSIBrokenRoadmarkCurve: maps to pending upstream RoadManager fix
    #     0403645c "Fix wrong s-value of tunnel OSI points" (not yet ported; separate RoadManager lineage).
    ...
    #   - OSI.TestTrafficLightStates: pending traffic-light OSI re-sync (separate from the sign/stationary
    #     rework ported here).
