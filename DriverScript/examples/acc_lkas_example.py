#!/usr/bin/env python3
"""
ACC + LKAS サンプルスクリプト（シンプル版）

ACCController（縦制御）と LKASController（横制御）を独立して使用し、
シナリオ上で先行車追従 + 車線維持を行うサンプルです。

- ACC: OSI-onlyモードで先行車を検出（座標変換ベース）
...
def main():
    ...
    # rm_lib=None: OSI-onlyモード（座標変換ベースで先行車を検出）
    print("Initializing ACC Controller (OSI-only mode)")
    acc = ACCController(ego_id=args.id)
