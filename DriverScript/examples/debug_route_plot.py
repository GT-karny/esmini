...
def main():
    ...
    parser.add_argument(
        "--lib_path",
        default=r"E:\Repository\GT_esmini\esmini\DriverScript\bin\esminiRMLib.dll",
        help="Path to esminiRMLib.dll",
    )
    parser.add_argument(
        "--gt_lib_path",
        default=r"E:\Repository\GT_esmini\esmini\DriverScript\bin\GT_esminiLib.dll",
        help="Path to GT_esminiRMLib.dll",
    )
    ...
    for wp in sparse_wps:
        ...
        if res == 0:
            ...
            print(
                f"  Fixed WP: Road={wp.road_id}, Lane={wp.lane_id}, S={wp.s:.2f}, X={wp.x:.2f}, Y={wp.y:.2f}"
            )

    print("\nGenerating Dense Route...")
    # Use the first waypoint as current position
    dense_wps = router.calculate_route_from_waypoints(
        current_pos=sparse_wps[0], waypoints=sparse_wps, step_size=1.0
    )
    ...
    for i, wp in enumerate(dense_wps):
        ...
        if i == 0 or i == len(dense_wps) - 1 or wp.lane_id != last_lane or i % 10 == 0:
            plt.text(
                wp.x, wp.y, f"L{wp.lane_id}", fontsize=6, color="darkgreen", alpha=0.8
            )
