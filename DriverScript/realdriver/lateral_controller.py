...
class LateralConfig:
    ...
    anticipate_min_heading: float = (
        0.1  # Minimum heading change to trigger anticipation (rad)
    )
...
class LateralController:
    ...
    def update_from_state(self, state: VehicleState, dt: float) -> float:
        ...
        curvature = self._calculate_curvature(
            wps, nearest_idx, cfg.curvature_sample_dist
        )
        ...
        curvature_factor = max(
            cfg.lookahead_min_factor, 1.0 - curvature * cfg.lookahead_curvature_scale
        )
        ...
        if target_idx >= len(wps) - 1:
            dist_to_end = math.sqrt(
                (state.x - wps[-1].x) ** 2 + (state.y - wps[-1].y) ** 2
            )
        ...
        speed_factor = max(
            cfg.xte_speed_min_factor,
            min(
                cfg.xte_speed_max_factor,
        ...
        steering = (
            cfg.heading_gain * heading_error + xte_correction + anticipate_steering
        )

        # === STEP 7: Apply smoothing ===
        curve_detected = self._curve_detected
        if (
            lane_change_like_segment
            or self._lane_change_state != LaneChangeState.LANE_KEEP
        ):
        ...
        steering = (
            steering * (1 - effective_smoothing)
            + self._last_steering * effective_smoothing
        )
        ...
        if self._debug_enabled:
            ...
            if self._log_counter % 20 == 0:
                antic_str = (
                    f"Antic={anticipate_steering:.3f}"
                    if anticipate_steering != 0
                    else ""
                )
                print(
                    f"[DEBUG_LAT] NearIdx={nearest_idx}, TgtIdx={target_idx}, "
    ...
    def _is_lane_change_like_segment(
        self, wps: List[Waypoint], nearest_idx: int
    ) -> bool:
        """
    ...
    def _calculate_curvature(
        self, wps: List[Waypoint], start_idx: int, sample_dist: float
    ) -> float:
    ...
    def _calculate_anticipatory_steering(
        self, wps: List[Waypoint], nearest_idx: int, speed: float, cfg: LateralConfig
    ) -> None:
        ...
        if (
            curve_start_dist is not None
            and abs(curve_heading_diff) > cfg.anticipate_min_heading
        ):
    ...
    def _lane_keep_steering(
        self, state: VehicleState, cfg: LateralConfig
    ) -> Optional[float]:
        ...
        res, info = self.rm_lib.GetLaneInfo(
            self._pos_handle, lookahead_distance=5.0, look_ahead_mode=0
        )
    ...
    def update_lane_change(
        ...
        steering_offset = (
            direction * 0.3 * math.sin(self._lane_change_progress * math.pi)
        )
