# VD テレメトリを OSI 拡張フィールドへ出すか — 実装しない判断の記録

> ステータス: **調査・設計のみ。実装しない。** 監査 doc §5 F5「OSI 拡張フィールドへの
> テレメトリ」の着手前判断材料として書かれ、**保留の結論が出たまま現在も有効**である
> （同じ棄却根拠を [`control_ownership_defects.md`](control_ownership_defects.md) §4 修正3-d が引く）。
> F5 の他項目(パス解決修正 / front-bumper テレメトリ / HVD velocity 移行 / dedup / ポート定数)は実装済み。
