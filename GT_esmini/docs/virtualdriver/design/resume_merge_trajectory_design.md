...
## 6-3. §6-2 の裏付け（凍結実機テレメトリ 6 本のログ解析）

§6-2 の決着走行より前に、凍結ログの解析だけで同じ結論に到達していた。
機構の説明はこちらにしかないので残す。
...
### ハーネスが車線の外まで押し出していた機構

ハーネスは復帰エッジで **`offset_at_edge = -3.230m`** を報告していた。
しかし `ego.offset` は走行車線上では半車線幅で頭打ちになるので、
`-3.230` は「隣の走行車線にいる」状態では出ない。
...
最寄り走行車線が -4 に張り付いた（clamp された）状態だけである。
§6-2 の決着走行が、まさにその状態（外端を 1.360m 越えた位置）を実測した。

**機構**: ハーネスの P 制御は目標を `|ego.offset| = 3.5` で指定している。この量は
**車線境界でリセットする座標**なので、境界を越えるたびに |offset| が小さく戻り、制御が
「まだ足りない」と外へ押し続ける。結果として**最後の走行車線を越えるまで止まらない**。
旧方式で最終 |offset| が 9.5–14m になった件も同じ機構で説明がつく。

条件の指定には、車線境界でリセットしない量（ルート車線中心からの横偏差＝`t` 系の量、
または車線幅を積算した量）を使う。具体的な再構成は §6-2-1 にある。
...
- `pos.GetLaneId()` は物理占有車線を追う（§6-2）。合流目標はルート車線とし（§2-0）、
  その引き方は §2-0-1 で確定した。
- オフセットの符号規約（§2-4）。`lane_sign` を流用しないことが要点である。

残っているもの:

- **`client.ts` の凍結契約**（編集点 9）。GUI 露出はユーザー要求に明記されているが、
  editor 側リポジトリはこのツリー外で同期できない。PM 判断。
- **実機 FFB マージンへの影響は未測定**。ヘッドレスで良化しても実機で確認するまで断定しない。
- **単発走行の実機マージンは判断材料にならない**（同一条件で 2.12x→1.41x と振れた実績）。
  実機で語るなら反復必須。
