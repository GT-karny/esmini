...
> 閾値2つを実測から確定した**（同）。
> **フェーズD（LKA/LDW、2026-08-05）実装済み**: `LaneKeepAssist`（§5、新設の本体。TLC 主判定＋
> margin 従・補正則・横 envelope・warning_only の判定/出力分離・指示器/操舵レートの2抑制・
> 速度域）、`gt.lka` / `gt.ldw` の HVD 行（§8-2）、DriverOverride の**ステア経路 producer**
> （§8-3。これで REQ-AD-028 段b の3経路 accel/brake/steering が全部そろい **met へ昇格**）。
> フェーズD実測: C++ ユニット **771/771 緑**（フェーズD新規 40）、`manualdrive_adas_batch` は
> **30 シナリオ・82 matcher が pass=82/fail=0/skip=0**、matcher 単体（Python）90 緑、
> 既存回帰ベースライン5本（計31シナリオ）は deviations=0 で不動、ODR 適合 quick 緑。
...
**★2026-08-05 訂正（実装で置き換え）**: 後半の「オフセットが直接閾値を超えたら」は、実装では
**margin に対する閾値**として書いた（`margin_threshold_m`、解除は `release_margin_m`）。
同じ量の言い換えではあるが（固定のレーン幅・車幅では定数差）、margin の次元で持つ理由が2つある。

1. **要求が問うている次元がそちらだから**。REQ-AD-027 は線を越えないことを問うており、
   車体の側面から線までの距離がその距離である。生オフセットの閾値が同じ主張になるのは、
   道路と車両が変わらない間だけである。
2. **TLC が既に margin から作られているから**（`TLC = margin / |横速度|`）。margin 閾値と
   TLC 閾値は1つの幾何量に対する2条件になり、別々の量に対する2条件のように食い違わない。

**この置き換えが実際に何を防いだか（当初値が到達不能だった実例）**。`straight_500m_2lane.xodr`
のレーン幅は 3.07 m、検証車両の幅は 2.00 m なので、**中心での margin は 0.535 m しかない**。
設計時に置いた既定値（engage 0.30 / release 0.50）は、解除条件が「中心 ±3.5 cm」を意味する
——ヒステリシスが黙ってラッチに化けていた。0.15 / 0.30 へ直して実帯域を作った。
生オフセットのままだったら、この不整合は「レーン幅を見る」という一手を挟まないと気づけない。
...
**★2026-08-05 追加（フェーズD）: 「妥当そうな丸い数字」を、それが載る物理量を見ずに置くと、
閾値は間違うのではなく到達不能になる**。LKA は既存 policy と閾値を1つも共有しない完全新設で、
既定値をすべて自前で置いた。走らせる前に**3つとも直した**——そして3つとも、症状は
「値を振っても何も起きない」だった（§12 冒頭の FCW ペア欠落と同じ型）。

| 当初値 | 直した値 | 到達不能だった理由 |
| :--- | :--- | :--- |
| margin 0.30 / release 0.50 | 0.15 / 0.30 | レーン幅 3.07 m・車幅 2.00 m → 中心での margin は 0.535 m。解除条件 0.50 は「中心 ±3.5 cm」＝ヒステリシスがラッチに化けていた |
| kp 0.06 / kd 0.25、上限 0.08 | 0.012 / 0.020、上限 0.03 | steer_gain 0.61 のこの車では、25 m/s で 0.3 m/s の横流れを消すのに必要な正規化操舵は **0.0024**。補正は 0.01 の桁であって 0.1 ではない。1桁以上大きい上限は「補助」ではなくスナップと振動を作る |
| 操舵入力レート 0.35 /s | 0.03 /s | 通常の車線変更の入力レートは約 0.01 /s（3 m を 3 s で移るのに必要な正規化操舵が約 0.010）。0.35 /s は 35 倍で、通常運転では**一度も跨がない**＝検出器として死んでいた |

**一般化**: 閾値を置くときは、その量が**実際にどんな値を取るか**を先に1つ計算する。
レーン幅なら road を見る、補正操舵なら steer_gain と要求される yaw rate から逆算する、
入力レートなら「その操作を人がやると何 /s になるか」を出す。
この一手が無いと、置いた値は「大きすぎて常時 ON」か「小さすぎて永久 OFF」のどちらかに落ち、
**どちらも実行はできて緑にもなる**（本フェーズでは margin の詰みだけが挙動に出て、
残り2つは実走前に算術で捕まえた）。

**★2026-08-05 追加（フェーズD）: 上書きチャネルの producer 条件は「入力」であって「幾何」ではない**。
§8-3 がフェーズBでアクセル経路について書いた規則（`kickdown_effective` であって
`aeb_suppressed` ではない）は、ステア経路でも**そのまま**成り立つ。ここでは実データが
それを再現した: 最初の実装は `suppressed_steer && departing` という狭い条件で、
`md_lka_human_steer` で運転者が道路を横切って操舵している最中、車両がたまたま通過中の
車線の中心に来た**1フレーム**（t=7.30、offset 0.115 m）だけ逸脱判定が false に落ち、
上書き欄が 50 ms だけ消えた——運転者の手は前後で何一つ変わっていないのに。
`driver_override_steering = suppressed_steer` へ直した。狭い事実（そのフレームに逸脱が
立っていたか）は `gt.lka.departure` として引き続き観測できる。

**★2026-08-05 追加（フェーズD）: 後段の整形ステージが前段の安全上限を押し戻せてはいけない**。
横 envelope は「振幅クランプ → レート制限」の2段で書いた。レート段は `prev` へ向かう収縮
なので振幅上限は保たれる、と最初は考えた——`prev` が**現在の**上限の内側にある間だけ正しい。
`correction_max` を 0 に絞る（判定は動かしたまま出力だけ止める）と、`prev`=0.5 に対して
レート段が結果を 0.48 へ引き戻し、「上限」が前フレームの値になった。
**これは `AdSteeringEnvelope` のジャーク段が shipped-disabled のままである当の非対称と同じもの**
（あちらは曲率クランプの後でジャーク窓が指令を a_lat 上限の外へ押し出す）。
ここでは修正できた: 振幅は硬い権限上限、レートは快適性なので、衝突したら振幅が勝つ
——「振幅 → レート → **振幅**」の3段にした。通常フレームでは2度目のクランプは no-op で、
レート制限は完全に効いたままである。

**★2026-08-05 追加（フェーズD）: 抑制は即時停止であって、ランプダウンではない**。
補正の終わり方は2つあり、混ぜてはいけない。
(a) **抑制**（指示器・操舵入力）: REQ-AD-027 段b の語は**即時中断**で、その場で 0 にする。
レート制限で落とすと、「もう自分で操舵する」と宣言した運転者に対して数フレーム押し続ける。
(b) **逸脱判定の終了**（車両がヒステリシス帯へ戻った）: こちらは誰も異議を唱えていないので
レート制限で滑らかに手を離す（1フレームで寄与を落とすと運転者には離した瞬間のショックになる）。
最初の実装は両方を (b) で書いており、ユニットテストが 0.020 の残留を捕まえた。
