...
#### Phase 3d — 対向車待ち（右折時）★本丸（2〜3 週）

**成果物**:
- `virtualdriver/policies/ConflictPointResolver.{hpp,cpp}`
- 自分の経路（waypoints）と他車の予測経路（観測 + 簡易外挿）の交差点を s 座標で算出
- TTC 計算 → 待機 or 進入判定
- 他車予測: 他車に Route があれば利用、無ければ「現レーン直進」仮定
- 国別ルール: LHT/RHT 切替（`opendrive-lht-rht.md` 参照）

**受入基準**:
- 信号無し交差点で右折する時、対向直進車を待つ
- ギャップが十分ならスムーズに通過
- 何台か通過後に余裕タイミングで進入

**リスク**: 高。他車意図予測の精度、TTC 閾値、待機解除条件などのチューニング地獄。
...
| Phase 3d 開始 | LHT/RHT 切替方法（JSON / xosc property / 自動推定） | 国際対応 |
