...
| **B2 マニューバ発起** | FUNC-055..060 | layer 軸で初めて分離できた欠陥。**2026-08-02 に一部解消**: FUNC-055 の経路要求 LC は自発発起できるようになった（vd-component:lane-change-initiation、req-vd-ad:REQ-AD-017 段c、**既定 OFF**）。残る欠陥は FUNC-056..060（追い越し/交差点/発進/合流）と、FUNC-055 のうち遅い先行車・専用レーン回避を動機とする発起 |
| **B3 意図伝達** | FUNC-048, 061..063 | **2026-08-02 に縦串が通った**: FUNC-061 が AD 発起 LC に同期し、発起より前から先行点灯するようになった（req-vd-ad:REQ-AD-018 段a、matcher:indicator_leads_lane_change、実測リード 0.40s/4.45s）。残る欠陥は法定 3 秒の保証（加速中に縮む＝段 b/c）と FUNC-056..059 未同期（FUNC-061 partial） |
...
**初回実測（88件・2026-07-21 実装時点のスナップショット）**: ④(a)未emit 15 ／ ④(b)未配線 19 ／
台帳の自己矛盾 1 ／ ⑥常設欠 7 ／ ②刺激欠 25 ／ coupling 21。
**⚠ この数字は動く**（動くのが目的の台帳）。同日中に AEB 常設化と `_gt_to_scene` 配線で
82 → 79 まで減った実測がある。**現在値は必ず `--spine-report` を回して読むこと**。
ここに固定値を書き直さない（陳腐化する実数を doc に置かない — CLAUDE.md「25 unit sources」の轍）。
