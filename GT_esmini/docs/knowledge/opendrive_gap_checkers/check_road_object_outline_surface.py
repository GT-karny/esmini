...
def run_checks(file_path, root, roads, road_ids, junctions, junction_ids):
    ...
    for rid, r in roads.items():
        ...
        for obj in r.iter("object"):
            ...
            if has_outline_like:
                for rep in repeats:
                    bad = [a for a in ("lengthStart", "lengthEnd", "widthStart", "widthEnd") if rep.get(a) is not None]
            ...
            if obj.get("radius") is not None:
                for rep in repeats:
                    bad = [a for a in ("widthStart", "widthEnd") if rep.get(a) is not None]
                    if bad:
                        flags.append((
                            "road.object.repeating.no_widthstart_end_with_radius",
            ...
            if surf is not None:
                ...
                if len(crgs) > 1:
                    crg_files = sorted({c.get("file") for c in crgs if c.get("file") is not None})
                    flags.append((
                        "road.object.surface.only_one_crg_file",
