...
def run_checks(file_path, root, roads, road_ids, junctions, junction_ids):
    ...
    for label, ids in (
        ...
        ("junction", [e.get("id") for e in root.iter("junction") if e.get("id") is not None]),
        ("junctionGroup", [e.get("id") for e in root.iter("junctionGroup") if e.get("id") is not None]),
        ("controller", [e.get("id") for e in root.findall("controller") if e.get("id") is not None]),
    ):
    ...
    controller_ids = {c.get("id") for c in root.findall("controller") if c.get("id") is not None}
    signal_ids = set()
    object_ids = set()
    ...
    for rid, r in roads.items():
        ...
        if jid and jid != "-1" and jid not in junction_ids:
            flags.append((
                "ids.only_ref_defined_ids", f"road {rid} @junction={jid} が未定義", f"road {rid}",
            ))
        link = r.find("link")
        if link is not None:
            for tag in ("predecessor", "successor"):
                ...
                if et == "road" and eid not in road_ids:
                    flags.append((
                        ...
                        f"road {rid}.link.{tag} -> road {eid} が未定義", f"road {rid}",
                    ))
                elif et == "junction" and eid not in junction_ids:
                    flags.append((
                        "ids.only_ref_defined_ids",
                        f"road {rid}.link.{tag} -> junction {eid} が未定義", f"road {rid}",
                    ))
    ...
    for jid, j in junctions.items():
        ...
        if main_road is not None and main_road not in road_ids:
            flags.append((
                "ids.only_ref_defined_ids",
                f"junction {jid} @mainRoad={main_road} が未定義road", f"junction {jid}",
            ))
        for conn in j.iter("connection"):
            ...
            for attr in ("incomingRoad", "connectingRoad", "linkedRoad"):
                ...
                if v is not None and v not in road_ids:
                    flags.append((
                        "ids.only_ref_defined_ids",
                        f"junction {jid} connection.{attr}={v} が未定義road", f"junction {jid}",
                    ))
        ...
        for pr in j.findall("priority"):
            for attr in ("high", "low"):
                ...
                if v is not None and v not in road_ids:
                    flags.append((
                        "ids.only_ref_defined_ids",
                        f"junction {jid} priority.{attr}={v} が未定義road", f"junction {jid}",
                    ))
        for jc in j.findall("controller"):
            ...
            if v is not None and v not in controller_ids:
                flags.append((
                    "ids.only_ref_defined_ids",
                    f"junction {jid} controller id={v} が未定義controller", f"junction {jid}",
                ))
    ...
    for c in root.findall("controller"):
        ...
        for ctrl in c.findall("control"):
            ...
            if v is not None and v not in signal_ids:
                flags.append((
                    "ids.only_ref_defined_ids",
                    f"controller {cid} control.signalId={v} が未定義signal", f"controller {cid}",
                ))
