        ...
        for obj in objs:
            ...
            for outline in _outlines_of_object(obj):
                ...
                if corner_locals and len(corner_locals) < 2:
                    flags.append((
                        "road.corner_local.element_min_amount",
                        "cornerLocal が{}個（outline内に2個以上必要）".format(len(corner_locals)),
                        oloc,
                    ))
                if corner_roads and len(corner_roads) < 2:
                    flags.append((
                        "road.corner_road.element_min_amount",
                        "cornerRoad が{}個（outline内に2個以上必要）".format(len(corner_roads)),
                        oloc,
                    ))
                ...
                if len(kinds_present) > 1:
                    detail_parts = []
                    for n, lst in (("cornerRoad", corner_roads), ("cornerLocal", corner_locals),
                                   ("curveLocal", curve_locals)):
                        if lst:
                            detail_parts.append("{}x{}".format(n, len(lst)))
                    flags.append((
                        "road.corner_road.corner_road_local_exclusivity",
                        "同一outlineに複数種の輪郭要素が混在: " + ", ".join(detail_parts),
                        oloc,
                    ))

                # --- mandatory_id_with_markings: every corner needs @id when <markings> present ---
                if markings is not None:
                    missing_cl = [i for i, c in enumerate(corner_locals) if c.get("id") is None]
                    if missing_cl:
                        flags.append((
                            "road.corner_local.mandatory_id_with_markings",
                    ...
                    missing_cr = [i for i, c in enumerate(corner_roads) if c.get("id") is None]
                    if missing_cr:
                        flags.append((
                            "road.corner_road.mandatory_id_with_markings",
