...
> ステータス: **S0 完了（足場・計測）、S1 以降は未実装**。本書は「規格が何を要求しているか」と「GT が今どこまで
...
**動的側は変わらない。** 論理レーン面は静的 GroundTruth にしか載らない。毎フレーム流れる外部
GroundTruth は実測 2.1 KB で、ここに増えるのは `logical_lane_assignment`（§2-5、1 オブジェクト
あたり Identifier + double 3 本 ≒ 35 B、20 台で約 0.7 KB/フレーム）だけである。GroundTruth の
UDP はチャンク済みなのでここは問題にならない。**上限に当たるのは HVD 側の `route` だけ**（§5-4）。
...
1 と 3 は設計書で方針を示した。2・4・5・6 は着手時にユーザー判断が要る
（設計書 §11 に判断材料つきでまとめてある）。
