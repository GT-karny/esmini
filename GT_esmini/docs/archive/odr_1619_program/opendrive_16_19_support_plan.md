...
| **P7** | ✅ 完了 (2026-07-04、feature/odr1619-p7、**dev マージ済 114d9e5f**) | 9e52f82d, 648c4db8, 5ce441eb, ff6b7ce8, cd27b18e, c8ec76e7, dd0b8b83, 0987a781, 48925ab5, 47cb8a32, 95406594 | クラスタ19a/19b/17/18/8/9(オブジェクト/断面ジオメトリ第1弾): **[GT_ODR:curvelocal]×2**(1.9 `<curveLocal>` アウトラインを既存 OutlineCornerLocal へ弧長テッセレート委譲=`gt_esmini::odr::AppendCurveLocalCorners`、加えて上流が読まない**単数 `<object><outline>` 形の受入**=g4 が使う形、複数形資産は `next_sibling("outline")` が無名反復と等価でビット同一)+**[GT_ODR:repeat-cubics]×1**(離散 repeat の @bT/@cT/@dT 横三次 + detachFromReferenceLine を `AdjustRepeatInstancePose` で適用、横多項式なしのレガシーは即 false・s/t 無改変=ビット同一)、フォーク計 **90/150 行**(P7 追加 15=curvelocal 12 + repeat-cubics 3)・マーカー20。**OdrObjectExtras**(19a: outline 属性/markings/skeleton/borders、repeat 横多項式、19b: objectReference/bridge/material/@perpToRoad L1)+**合成**(objectReference→原オブジェクト複製で stationary count≥2、bridge=tunnel 経路並行の L1+OSI StationaryObject TYPE_BRIDGE=2、synth id 基底 objectReference=920000000/bridge=910000000)、**OdrJunctionGeom**(クラスタ8 junction `<boundary>`/`<surface>` L1 + 9 junctionGroup L1)、**shape/crossSectionSurface→superelevation 明示劣化**(クラスタ17: L1 格納 + θ=atan(b) 等価スーパーエレベーションへ WARN 付き劣化、ネイティブ t 依存 z 評価は保留、z_probe が t=±3 で z=±0.059988 を解析式 rm_expect_z と一致で機械検証)、**CRG L1+ファイル存在診断**(クラスタ18、評価なし)、**旗付き L3=authored junction boundary→OSI 交差点輪郭**(クラスタ8/9: `GT_ODR_OSI_AUTHORED_JUNCTION_BOUNDARY` **デフォルト OFF**、ON 時のみ heuristic free-lane-boundary を authored 輪郭 1 本に差替え、OFF は完全 no-op で OSI ゴールデンバイト同一 — probe で OFF=[29,37,47,59]/ON=[62]4pt を機械検証)。ハーネス拡張: z-grid probe + stationary base_polygon dump(opt-in)。フィクスチャ 23/24/25/26+g8 追加(手書き 26 まで+生成 g8)、ゴールデン新規 9 本のみ(RM 23/24/25/26/g8 + OSI g4/10/11/26、既存全件は `--update-golden` 後も CRLF ゴーストのみ=回帰ゼロ)、conformance full **233P/0F/13XF/0XP**(quick 191P・smoke 3/3)、unit ctest 163/163、GT_RoadGen 公式 8 資産 exit0/.osgb>4KB、コントロール 3 資産 .osgb dev_v0.12 とバイト同一(0.0000%)、回帰ゲート(Step1/1.5 緑・Step2 は既知 TL 2件のみ fail=dev 基準一致・他 9 件 pass)、validate_catalog 61/61、7 フィクスチャの odrviewer スクリーンショット確認(bridge span / objectReference 2本 / object_details ボックス / g8 交差点 描画確認、23/24 の断面傾斜は微小で既定アングルでは非可視) |
| **P8** | ✅ 完了 (2026-07-04、feature/odr1619-p8、**dev マージ済 4a08c403**、base=P7マージ済 114d9e5f) | S1 lane-layers コア / S2 invalidated 消費者 / S3 ハーネス+フィクスチャ / ゴールデン+docs | クラスタ4+22意味論(最後のサイレント欠落クラス=第2 `<lanes layer>` の解消): **[GT_ODR:lane-layers]×1・フォーク追加 2 行のみ**(計画見込み ~22 → 実績 2、計 **92/150**・マーカー21) — `child("lanes")` → `SelectLanesLayer(road_node, this)` の薄いデリゲート、選択/マージ全量は GT 側 `odr_side/OdrLaneLayers.cpp`(新規、既存 [GT_ODR:cmake] APPEND リスト+1 行=マーカー数不変)。**permanent モード(デフォルト)=無コピー同一ノード返却**(AddLane/SetGlobalId の DOM 反復順不変 → グローバル ID 安定が構造的に自明)/ **temporary opt-in**(env `GT_ODR_LANE_LAYERS=temporary`、プロセス毎ラッチ=D1)= laneSection/laneOffset の s 範囲マージ合成 DOM(境界規則=t1 で Taylor シフト再アンカー(D2)、両公式資産は境界一致で再オープン経路不発=純ロジック単体テストで担保、合成 DOM 寿命はサイドモデル所有(D3))。ParseLaneExtras は同一マージビューを歩く(D6)。L1: OdrRoadLaneLayers シャドウ(レイヤ/セクション@length/temporary被覆範囲)+lane link @layer+validity@layer+signal/object @temporary/@invalidated(全て sparse、junction laneLink fromLayer/toLayer は P5 スロットで捕捉済み)。**invalidated 意味論**: OSI TrafficSign/TrafficLight/StationaryObject から除外(D4、temporary は情報のみ)+ VD は RouteSignalScan 1 箇所フィルタで StopYield/TrafficLightAware 両ポリシーをカバー(D5)。ハーネス: `rm_env` / `golden_suffix`(同一 path 変種)/ `rm_lane_s_samples`(additive `lane_samples_at` キー=バイト同一ガード)+ `osi_expect_traffic_sign_count`。**受入実測**: (i) MLL lane -3 @s=50 perm 3.75 / temp 3.625・roadworks レーン数 13→5/6/4→13(**s=100/5500=マージ証明点で permanent 値一致**)/ (ii) 両 rev9 公式+g2+06 で [ODR-UNSUPPORTED]==0・要素+属性(ホワイトリスト 659→676、g1 の laneSection@length フリップ含む)/ (iii) 新設 `golden/lane_global_ids.json`(オフセット正規化)+regen 後の実変化=基底 2 本の additive キーのみ・他全ゴールデン CRLF ゴーストのみ=ID 完全不変 / (iv) OSI sign_count=1(invalidated 非出力)+挙動 `invalidated_stop_sign_ignored` PASS・`semantic_stop_sign_full_stop` PASS 維持=過剰フィルタ無し / (v) GT_RoadGen .osgb **バイト同一**(fabriksgatan 325129B・roadworks 2858833B、pre-P8 exe 比較=二重テッセレーション無し)/ (vi) ALL ビルド 0 error・unit 173/173・conformance full **238P/0F/13XF/0XP**・回帰ゲート PASS(挙動 10 pass/既知 TL 2 件のみ fail=dev 基準一致)・validate_catalog 全数・smoke 3/3。設計判断 D1-D6 の全文は [gt_roadmanager_patches.md](gt_roadmanager_patches.md) §8 |
| **P9a** | ✅ 完了 (2026-07-04、merge 88af7d06) | ebac508d ほか | クラスタ20+15: railroad/station L1(不活性明記)+include恒久ハードエラー仕様化+GT_RM JSON API+web ODRパネル公開(レイヤ/VJ情報はP9bへ) |
| **P6** | ✅ 完了 (2026-07-05、**merge 14499123**) | f01a577f〜2231d9f4(19コミット、詳細台帳=[odr_p6_virtual_junction_design.md](odr_p6_virtual_junction_design.md)+[gt_roadmanager_patches.md](gt_roadmanager_patches.md) §7) | **virtual junction コアネイティブ実装**(R1第2種緩和・in-place編集): S0 ガバナンス(機械可読第2種manifest+二側センサス+レッドテスト+motion/telemetryゴールデンオラクル)→ S0b upstream **v3.4.0マージ**(静粛性を全オラクル証明)+[GT_ODR:osi-path] R1第3例外(v3.4.0のOSI 3.7.0黙示ダウングレード修正)→ S1 hpp加算 74/75 → S2 パース(fixtures 23/23b/23c、overlap残差3)→ S3 合成+レジストリ+membership(-1/false v1)+IsOsiIntersection=false → S4 path/route(Calculate起点シーディング+SetTrackSクランプ、実現性メジャー2件)→ S5 motion(MoveAlongSアンカー窓+中間離脱/進入+lockOnLane、T2パススルー不変)→ S6 LaneIndependentRouter 127/220(FollowRoute_test 20/20)→ S7 OSIレーンペアリング(anchor断面でmain global id解決)+looming 8/10+AutoLight → S7b VDシーン(分岐実走証明+junction-idアサーション禁止則)→ S8 PRパッケージ準備。**dev(P7+P8+P9a)リコンサイル込み**(f54cde34、fork第1種100/150・drift 94)。予算: hpp 74/75 / pristine cpp 530/550 / router 127/220 / Looming 8/10 / fork 100/150。ゲート: conformance FULL 349P/0F/0XP・ctest 217・RoadManager_test 126/126・telemetry全PASS・挙動=pre-P6既知2件のみ。**upstream PR-A〜D はローカル準備のみ(pr/vj-a..d、131/131緑)— 2026-07-05 ユーザー決定により非提出**(§7 PR-VJ行・レビュー地平条項は不発動、資産はブランチ+upstream_pr/保管) |
| **P9b** | ✅ 完了 (2026-07-05、**merge ccb42e65**) | d5745354〜6996e15c(10コミット) | **全マージ後の締め=プログラム終了判定達成**: ゼロ監査(conformance full --check-matrix **350P/0F/13XF/0XP**・要素+属性 [ODR-UNSUPPORTED]==0・行列全行充足。最後のpinned属性 road/type@country を OdrRoadTypeExtra L1格納で解消、manifest残=removed16_neighbor のみ=by-design恒久台帳)/ レーンレイヤ+VJメタデータweb API(GT_RM_GetLaneLayersJson+GetVirtualJunctionsJson→REST lane_layers/virtual_junctions→UIバッジ+スパンマーカー、証跡 docs/images/p9b_*)/ **WASMリンク修理=全機能ブラウザ点灯**(P6未実施のem++ゲートを先行実施: 14TU全緑=予算増分ゼロ、odr_sideを[GT_ODR:cmake]APPENDリストからconfigure時抽出=ドリフト不能、リンク71/71、手動ブラウザスモークPASS=web/wasm/smoke/index.html、台帳wasm除外解消)/ **resyncリハーサル**(乾式再適用 7ファイル/126ハンク全一致、書面=odr_resync_checklist.md、常設ガード=check_resync_guards.py(conformance内蔵)+ctest OdrResyncGuards(合成ID 900M/910M/920M重複・再パース蓄積ゼロ))/ 最終ステータス表 **opendrive_16_19_support.md**(再開情報付き保留台帳)/ dat.py v5明示エラー化(HTTP 409+SystemExitサーバ死防御)。**フォーク・第2種予算完全不変**(EnvironmentSimulator/とフォークのdiffゼロ)。ゲート: ALLビルド0error・unit ctest緑・回帰ゲートPASS(挙動fail=既知2件のみ)・validate_catalog全数・スモーク3/3 |
| F3週 / P10 | 未着手(計画対象外へ) | — | **F3週**(junction優先権: GetJunctionPriorities消費+ConflictPointResolver Evaluate改修、無期延期中 — 再開情報は opendrive_16_19_support.md §3-9)/ P10 upstream還元(PR-VJ非提出決定によりR4アップサイドは縮小、要再評価) |
...
- **承認済**(2026-07-02〜05): §10-1 CMake swap-zone R1例外 / §10-2 フォーク150行上限 / §10-7 ASAM資産=**テスト時zip展開・展開物はコミットしない** / §10-3・4=P6ネイティブ再設計で無効化 / §10-4b P6第2種予算(2026-07-04) / §10-6=**P9aでinclude恒久ハードエラー仕様として確定済**。**§10-5 承認済(2026-07-05)**: §8保留台帳を [opendrive_16_19_support.md](opendrive_16_19_support.md) の最終ステータス表(項目別再開情報付き)でレビューし、「後日再開に十分な情報がある」条件付きで承認 → 同表 §3 に全項目の再開情報(データ所在→次の一手→見積)を明記して条件充足。**未承認事項なし=プログラム終了**。
- **実装時の実測差分(計画からの補正)**:
...
3. **【2026-07-04スコープ内へ昇格】virtual junctionネイティブ実装はP6本体になった**(R1第2種緩和の承認による)。旧保留理由(「junctionは道路端」不変条件をMoveAlongS/RoadPathで壊し、pristineなLaneIndependentRouter.cpp編集を強いる)は、in-place第2種緩和の承認で解消。v1で保留する下位項目は設計書§9参照: kind-2(topological)接続のルーティング使用 / orientation方向フィルタ強制 / main-road span上のjunction-id報告(upstreamへ提起) / VJアンカーでのランダム分岐 / lockOnLane完全対応。
