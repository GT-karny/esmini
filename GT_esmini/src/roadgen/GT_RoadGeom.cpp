...
// (upstream base: upstream/master @ d7be85d6, "Prepare release 3.4.1" line).
// GT deltas: multithreaded per-road surface generation (std::thread pool, GT-PARALLEL
// markers) for the standalone GT_RoadGen executable, plus GT-FIX robustness guards
// against non-terminating section loops. Re-sync against upstream on merges; see
// GT_esmini/docs/fork_sync_manifest.yaml (lineage: roadgen).
          ...
          optimize_(optimize)
    {
        ...
        if (generate_road_surface)
        {
            ...
            // friction-correct. Mirrors the material branches in the worker body below.
            {
                ...
                for (int ri = 0; ri < static_cast<int>(odr->GetNumOfRoads()); ri++)
                {
                    roadmanager::Road* rd = odr->GetRoadByIdx(ri);
                    for (int sj = 0; sj < static_cast<int>(rd->GetNumberOfLaneSections()); sj++)
                    {
                        ...
                        for (unsigned int li = 0; li < ls->GetNumberOfLanes(); li++)
                        {
                            ...
                            {
                                prewarm_frictions.push_back(ln->GetMaterialByIdx(static_cast<unsigned int>(mi))->friction);
                            }
                        }
                    }
                }
