 ...
 *   from upstream esmini v3.3.0 (upstream commit ab7c404d "Prepare release 3.3.0"). Vanilla
    ...
    for (pugi::xml_node road_node : node.children("road"))
    {
        ...
        if (signals != NULL)
        {
            ...
            for (pugi::xml_node signal = signals.child("signal"); signal; signal = signal.next_sibling())
            {
                if (!strcmp(signal.name(), "signal"))
                {
                    ...
                    if (!signal.attribute("countryRevision").empty())  // [GT_ODR:country-rev] presence-fix (upstream read only when absent)
                    {
        ...
        if (objects != NULL)
        {
            for (pugi::xml_node object = objects.child("object"); object; object = object.next_sibling("object"))
            {
                ...
                {
                    for (pugi::xml_node outline_node = outline_container.child("outline"); outline_node;  // [GT_ODR:curvelocal] iterate container (outlines wrapper or bare object)
