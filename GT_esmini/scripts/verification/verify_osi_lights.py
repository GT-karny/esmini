                    ...
                    if obj.type == 2:  # TYPE_VEHICLE
                        ...
                        if obj.HasField("vehicle_classification"):
                            ...
                            if vc.HasField("light_state"):
                                ...
                                print(
                                    f"    Brake Light:     {ls.brake_light_state} ({get_brake_light_name(ls.brake_light_state)})"
                                )
                                print(
                                    f"    Indicator:       {ls.indicator_state} ({get_indicator_name(ls.indicator_state)})"
                                )
                                print(
                                    f"    Reversing Light: {ls.reversing_light} ({get_generic_light_name(ls.reversing_light)})"
                                )
                                print(
                                    f"    Head Light:      {ls.head_light} ({get_generic_light_name(ls.head_light)})"
                                )
                                print(
                                    f"    High Beam:       {ls.high_beam} ({get_generic_light_name(ls.high_beam)})"
                                )
                                print(
                                    f"    Front Fog:       {ls.front_fog_light} ({get_generic_light_name(ls.front_fog_light)})"
                                )
                                print(
                                    f"    Rear Fog:        {ls.rear_fog_light} ({get_generic_light_name(ls.rear_fog_light)})"
                                )
                            else:
                                print(f"  Vehicle ID {obj_id}: No light_state field")
                        else:
                            print(
                                f"  Vehicle ID {obj_id}: No vehicle_classification field"
                            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Verify OSI light state output via UDP"
    )
    parser.add_argument(
        "--host", default="127.0.0.1", help="UDP host (default: 127.0.0.1)"
    )
    parser.add_argument(
        "--port", type=int, default=48198, help="UDP port (default: 48198)"
    )
    parser.add_argument(
