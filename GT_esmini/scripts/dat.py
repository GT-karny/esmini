...
class DATFile:
    ...
    def check_header(self, filename: str) -> None:
        ...
        if ret != 0:
            ...
            if old_header != -1:
                print(
                    f"ERROR: Old DAT file version {old_header} found which is not supported."
                )
            else:
        ...
        if self.version_major != VERSION_MAJOR:
            print(
                f"ERROR: Incompatible DAT major file version: {self.version_major}, supporting: {VERSION_MAJOR}"
            )
            exit(-1)

        if self.version_minor != VERSION_MINOR:
            print(
                f"Warning: DAT-file has version {self.version_major}.{self.version_minor}. Some inconsistencies are expected."
            )
        ...
        while self.file.tell() < self.file_size:
            ...
            if p_id == PacketId.TIMESTAMP.value:
                ...
                if (
                    ...
                    or self.current_timestamp > self.timestamps[-1]
                ):
                    self.timestamps.append(self.current_timestamp)
                    self.ghost_timeline_setup = False
                elif (
                    self.current_timestamp < self.timestamps[-1]
                    and not self.ghost_timeline_setup
                ):
            ...
            # OBJ_ID packet
                ...
                if self.objects_timeline.get(self.current_object_id) is None:
                    ...
                    self.current_object_timeline = self.objects_timeline[
                        self.current_object_id
                    ]

                    if self.current_timestamp > 0.0:
                        self.current_object_timeline.active.values.append([0.0, False])
                    self.current_object_timeline.active.values.append(
                        [self.current_timestamp, True]
                    )
                else:
                    self.current_object_timeline = self.objects_timeline[
                        self.current_object_id
            ...
            # POSE packet
                ...
                self.current_object_timeline.pose.values.append(
                    [self.current_timestamp, pose]
                )
                ...
                self.current_object_timeline.speed.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.WHEEL_ANGLE.value:
                self.current_object_timeline.wheel_angle.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.WHEEL_ROT.value:
                self.current_object_timeline.wheel_rot.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.POS_OFFSET.value:
                self.current_object_timeline.pos_offset.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.POS_T.value:
                self.current_object_timeline.pos_t.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.POS_S.value:
                self.current_object_timeline.pos_s.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.MODEL_ID.value:
                self.current_object_timeline.model_id.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.OBJ_TYPE.value:
                self.current_object_timeline.obj_type.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.OBJ_CATEGORY.value:
                self.current_object_timeline.obj_category.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.CTRL_TYPE.value:
                ctrl_type = read_dtype(self.file, DataType.int32)
                self.current_object_timeline.ctrl_type.values.append(
                    [self.current_timestamp, ctrl_type]
                )
                if ctrl_type == 100:
                    self.ghost_controller_id = self.current_object_id
            elif p_id == PacketId.SCALE_MODE.value:
                self.current_object_timeline.scale_mode.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.VISIBILITY_MASK.value:
                self.current_object_timeline.visibility_mask.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.ROAD_ID.value:
                self.current_object_timeline.road_id.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.uint32)]
                )
            elif p_id == PacketId.LANE_ID.value:
                self.current_object_timeline.lane_id.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.int32)]
                )
            elif p_id == PacketId.NAME.value:
                name = read_string_packet(self.file)
                self.current_object_timeline.name.values.append(
                    [self.current_timestamp, name]
                )
            elif p_id == PacketId.BOUNDING_BOX.value:
                bb = BoundingBox()
                for k in list(bb.__dict__.keys()):
                    setattr(bb, k, read_dtype(self.file, DataType.float))
                self.current_object_timeline.bounding_box.values.append(
                    [self.current_timestamp, bb]
                )
                ...
                self.current_object_timeline.refpoint_x_offset.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.MODEL_X_OFFSET.value:
                self.current_object_timeline.model_x_offset.values.append(
                    [self.current_timestamp, read_dtype(self.file, DataType.float)]
                )
            elif p_id == PacketId.OBJ_MODEL3D.value:
                model3d = read_string_packet(self.file)
                self.current_object_timeline.model3d.values.append(
                    [self.current_timestamp, model3d]
                )
    ...
    def setup_ghosts_timeline(self):
        ...
        if obj_tl is None:
            print(
                "ERROR: No ghost controller found even though a ghost restart was detected. Quitting."
            )
            exit(-1)
        ...
        if it is None:
            self.objects_timeline[self.ghost_ghost_counter] = copy.deepcopy(
                self.objects_timeline[self.ghost_controller_id]
            )
    ...
    def get_object_state_struct_at_time(self, obj_id: int, t: float) -> dict:
        ...
        self.ObjectStateStructDat["model_id"] = timeline.model_id.get_value_incremental(
            t
        )
        self.ObjectStateStructDat["obj_type"] = timeline.obj_type.get_value_incremental(
            t
        )
        self.ObjectStateStructDat["obj_category"] = (
            timeline.obj_category.get_value_incremental(t)
        )
        self.ObjectStateStructDat["ctrl_type"] = (
            timeline.ctrl_type.get_value_incremental(t)
        )
        ...
        self.ObjectStateStructDat["wheel_angle"] = (
            timeline.wheel_angle.get_value_incremental(t)
        )
        self.ObjectStateStructDat["wheel_rot"] = (
            timeline.wheel_rot.get_value_incremental(t)
        )
        self.ObjectStateStructDat["centerOffsetX"] = (
            timeline.bounding_box.get_value_incremental(t).center_offset_x
        )
        self.ObjectStateStructDat["centerOffsetY"] = (
            timeline.bounding_box.get_value_incremental(t).center_offset_y
        )
        self.ObjectStateStructDat["centerOffsetZ"] = (
            timeline.bounding_box.get_value_incremental(t).center_offset_z
        )
        self.ObjectStateStructDat["width"] = (
            timeline.bounding_box.get_value_incremental(t).width
        )
        self.ObjectStateStructDat["length"] = (
            timeline.bounding_box.get_value_incremental(t).length
        )
        self.ObjectStateStructDat["height"] = (
            timeline.bounding_box.get_value_incremental(t).height
        )
        self.ObjectStateStructDat["scaleMode"] = (
            timeline.scale_mode.get_value_incremental(t)
        )
        self.ObjectStateStructDat["visibilityMask"] = (
            timeline.visibility_mask.get_value_incremental(t)
        )
        ...
        self.ObjectStateStructDat["offset"] = timeline.pos_offset.get_value_incremental(
            t
        )
        ...
        self.ObjectStateStructDat["refpoint_x_offset"] = (
            timeline.refpoint_x_offset.get_value_incremental(t)
        )
        self.ObjectStateStructDat["model_x_offset"] = (
            timeline.model_x_offset.get_value_incremental(t)
        )
    ...
    def build_data(self, usage: str = "csv") -> None:
        ...
        if usage == "csv":
            ...
            raise ValueError(
                f"Unsupported usage type: {usage}, only supported are 'csv' and 'plot'."
            )
        ...
        if len(self.dt.values) == 1:
            ...
            while j < len(self.dt.values) - 1:
                ...
                if is_near(curr_time, (self.dt.values[j + 1][0] - next_dt)):
                    ...
                    start_idx -= 1
                    steps = int(
                        round(((end_time - start_time) / restart_dt) + SMALL_NUMBER)
                    )
                    for k in range(steps):
                        t = k * restart_dt + start_time
                        for m in range(start_idx, len(filled)):
                            if (
                                t > filled[m] + SMALL_NUMBER
                                and t < filled[m + 1] - SMALL_NUMBER
                            ):
                                filled.insert(m + 1, t)
                                start_idx = m + 1
                                break
                ...
                if (
                    curr_time + next_dt < next_timestamp - SMALL_NUMBER
                    and curr_time + dt < next_timestamp - SMALL_NUMBER
                ):
                    end_time = next_timestamp - next_dt
                    self.fill_empty_timestamps(curr_time, end_time, dt, filled)
                elif (
                    j == len(self.dt.values) - 1
                    and curr_time + dt < self.timestamps[-1] - SMALL_NUMBER
                ):
                    self.fill_empty_timestamps(
                        curr_time, self.timestamps[-1], dt, filled
                    )
                else:
    ...
    def fill_empty_timestamps(
        self, start: float, end: float, dt: float, v: List[float]
    ) -> None:
