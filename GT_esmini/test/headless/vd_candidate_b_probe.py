...
def run_push2_signature_probe(
    envelope_enabled: bool,
    push2_mag: float,
    push2_hold_s: float = 2.0,
    speed_mps: float = 8.0,
    target_offset_m: float = 1.3,
    push_mag: float = 0.5,
    push2_sign: float = 1.0,
    ramp_cap_s: float = 3.0,
    resume_wait_cap_s: float = 2.5,
    post_push2_s: float = 0.5,
    ...
    for f in seg2:
        ...
        target_norm = (
            (actual_norm + pos_err)
            if (actual_norm is not None and pos_err is not None)
            else None
        )
        ...
        trace.append(
            {
                ...
                "sign_opp": gates.get("sign_opposition"),
                ...
                "block_reason": reason,
                "lateral_latched": f["override"].get("lateral"),
            }
        )
    ...
    result = {
        ...
        "latched_2_final_frame": latched_2,
        "ever_latched_2_during_push2": ever_latched_2,
        "n_frames_push2": len(seg2),
        "block_reason_counts": reason_counts,
        "first_wheel_not_engaged_t_rel": (
            round(first_wne_t - seg2[0]["sim_time"], 3)
            if (first_wne_t is not None and seg2)
            else None
        ),
        "first_wheel_not_engaged_target_norm": (
            round(first_wne_target, 4) if first_wne_target is not None else None
        ),
        "trace": trace,
    }
