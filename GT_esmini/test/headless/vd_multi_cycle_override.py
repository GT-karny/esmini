...
from vd_resume_transient import (  # noqa: E402
    BASE_XOSC, BTN_AUTO_RESUME, DLL, DT, INPUT_PORT, MAGIC, WIRE,
    _load_lib, _make_variant,
)


def run_multi_cycle_network(domain: str, envelope_enabled: bool, speed_mps: float = 8.0,
                             n_cycles: int = 3) -> tuple[list[dict], list[dict]]:
    ...
    cfg = {"input_type": "network", "input_port": INPUT_PORT, "input_transport": "udp",
           "ffb_target_track_enabled": False, "ad_steering_envelope_enabled": envelope_enabled}
    xosc = _make_variant(tmpdir, cfg, speed_mps)

    lib = _load_lib()
    argv_list = [b"multicycle", b"--osc", xosc.encode(), b"--headless", b"--fixed_timestep", b"0.05"]
    ...
    for c in range(1, n_cycles + 1):
        ...
        seg = [f for f in frames if f["_phase"] in (f"c{c}_resume_pulse", f"c{c}_resume_settle")]
        ...
        cycle_results.append({"cycle": c, "manual_edge_seen": manual_edge, "latched": latched,
                               "held_after_release": held, "auto_edge_seen": auto_edge, "resumed": resumed})
...
def run_multi_cycle_ffb(envelope_enabled: bool, speed_mps: float = 8.0, frozen_at: float = 0.4,
                         total_s: float = 20.0) -> tuple[list[dict], list[tuple[str, float]]]:
    ...
    cfg = {"input_type": "headless_ffb", "ffb_target_track_enabled": True,
           "steering_threshold": max(1.0, abs(frozen_at) + 0.5), "auto_return_timeout": 1.0,
           "ad_steering_envelope_enabled": envelope_enabled}
    xosc = _make_variant(tmpdir, cfg, speed_mps)

    lib = _load_lib()
    argv_list = [b"multicycle_ffb", b"--osc", xosc.encode(), b"--headless", b"--fixed_timestep", b"0.05"]
    argv = (ctypes.c_char_p * len(argv_list))(*argv_list)
    rc = lib.GT_InitWithArgs(len(argv_list), argv)
    if rc != 0:
        raise RuntimeError(f"GT_InitWithArgs rc={rc}")
...
def run_frozen_grip_natural(envelope_enabled: bool, speed_mps: float = 8.0, frozen_at: float = 0.0,
                             total_s: float = 20.0) -> tuple[list[dict], list[dict]]:
    ...
    lib = _load_lib()
    argv_list = [b"grip_natural", b"--osc", xosc.encode(), b"--headless", b"--fixed_timestep", b"0.05"]
    argv = (ctypes.c_char_p * len(argv_list))(*argv_list)
    rc = lib.GT_InitWithArgs(len(argv_list), argv)
    if rc != 0:
        raise RuntimeError(f"GT_InitWithArgs rc={rc}")
    ...
    for f in frames:
        ...
        if ov.get("manual_transition") or ov.get("auto_transition"):
            edges.append({"kind": "manual" if ov.get("manual_transition") else "auto",
                           "t": f["sim_time"], "position_error": f.get("ffb", {}).get("position_error"),
                           "gates": f.get("ffb", {}).get("gates")})
    return frames, edges


PUSHBACK_PORT = 9105  # must match HeadlessFfbInput.cpp GT_HEADLESS_FFB_PUSHBACK_PORT default


def run_multi_cycle_pushback(envelope_enabled: bool, speed_mps: float = 8.0, n_cycles: int = 3,
                              pushback_mag: float = 0.5, push_hold_s: float = 0.6,
                              release_wait_s: float = 1.4) -> tuple[list[dict], list[dict]]:
    ...
    lib = _load_lib()
    argv_list = [b"multicycle_pushback", b"--osc", xosc.encode(), b"--headless", b"--fixed_timestep", b"0.05"]
    argv = (ctypes.c_char_p * len(argv_list))(*argv_list)
    rc = lib.GT_InitWithArgs(len(argv_list), argv)
    if rc != 0:
        raise RuntimeError(f"GT_InitWithArgs rc={rc}")
    ...
    for c in range(1, n_cycles + 1):
        ...
        dev_peak = max((abs(f.get("ffb", {}).get("position_error", 0.0)) for f in seg), default=0.0)
        ...
        cycle_results.append({"cycle": c, "manual_edge_seen": manual_edge, "latched": latched,
                               "dev_peak": round(dev_peak, 4), "auto_edge_seen": auto_edge, "resumed": resumed})
...
def run_pushback_recovery_timing(envelope_enabled: bool, push2_hold_s: float, speed_mps: float = 8.0,
                                  target_offset_m: float = 1.3, push_mag: float = 0.5,
                                  ramp_cap_s: float = 3.0, resume_wait_cap_s: float = 2.5,
                                  post_push2_s: float = 1.0) -> tuple[list[dict], dict]:
    ...
    lib = _load_lib()
    argv_list = [b"pushback_timing", b"--osc", xosc.encode(), b"--headless", b"--fixed_timestep", b"0.05"]
    argv = (ctypes.c_char_p * len(argv_list))(*argv_list)
    rc = lib.GT_InitWithArgs(len(argv_list), argv)
    if rc != 0:
        raise RuntimeError(f"GT_InitWithArgs rc={rc}")
    ...
    prev_out = None; prev_t = None
    over_rate_time = 0.0
    dev_vals = []
    for f in seg2:
        env_blk = f.get("envelope", {})
        t = f["sim_time"]; steer_out = env_blk.get("steer_out")
    ...
    result = {
        "envelope_enabled": envelope_enabled, "push2_hold_s": push2_hold_s,
        "offset_reached": reached, "offset_at_release": offset_at_release,
        "held_after_release": held_after_release, "auto_fired": auto_fired,
        "manual_edge_2_seen": manual_edge_2, "latched_2": latched_2,
...
def run_timing_sweep() -> int:
    ...
    for push2_hold_s in (0.6, 0.3, 0.2):
        for envelope_enabled in (False, True):
            ...
            print(f"  push2_hold_s={push2_hold_s} envelope_enabled={envelope_enabled}: {res}")
...
def main() -> int:
    ...
    for envelope_enabled in (False, True):
        ...
        print(f"  envelope_enabled={envelope_enabled}: manual_edges={manual_count} auto_edges={auto_count} "
              f"edges={edges}")

    print("== headless_ffb SCRIPTED pushback (requires HeadlessFfbInput 'pushback' mode) ==")
    out_dir = os.path.join(os.path.dirname(BASE_XOSC), "..", "..", "test_results", "vd_multi_cycle_override")
    out_dir = os.path.abspath(out_dir)
    os.makedirs(out_dir, exist_ok=True)
    for envelope_enabled in (False, True):
        ...
        for c in cyc:
            if c["cycle"] > 1 and not c["latched"]:
                ...
                for f in seg[:20]:
                    env = f.get("envelope", {})
                    print(f"      t={f['sim_time']:.2f} steer_in={env.get('steer_in')} "
