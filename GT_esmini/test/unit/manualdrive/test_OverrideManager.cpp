...
// --- feature:F7 (F7b) — velocity-opposition gate (envelope-ramp blackout) --
//
// Fifth-order regression (real-machine report, found after commit 93b2c6c4):
// "override → AUTO resume → second override never fires". The AD steering
// safety envelope ramps the wheel target at up to ~2.46 axis-frac/s during
// the post-RESUME recovery transient — comfortably above
// override_target_rate_gate (0.30) for the whole transient, so signature (A)
// (position opposition, gated by moving_target/tracking_transient) stays
// blacked out exactly when a driver is most likely to grab the wheel. Fix:
// signature (B), velocity opposition (commanded_force_signed vs
// d(actual_norm)/dt sign), is invariant to target motion and accumulates
// independently of the two rate gates.

TEST(OverrideManagerTest, FfbVelocityOppositionLatchesDuringMovingTargetRamp)
{
    // AD target ramps at 2.5 axis-frac/s (>> the 0.30 gate) for the whole
    // test — signature (A) is blacked out throughout. The driver
    // counter-steers: actual_norm moves OPPOSITE to the servo's signed force.
    // Signature (B) must still latch within sustain_time.
    OverrideManager m;
    m.Configure(MakeConfigWithFfbThresholds(0.20, 0.04, 0.10, 0.30, 0.10));

    FfbInterventionSample s;
    s.active                 = true;
    s.commanded_force_signed = 0.50;   // servo pushing "up"
    s.commanded_force        = 0.50;

    bool saw_manual_edge = false;
    for (int i = 0; i <= 15; ++i)
    {
        s.target_norm             = 0.05 * i;          // rate 2.5/s >> 0.30 gate
        const double actual_norm  = 0.5 - 0.10 * i;     // rate -5.0/s, opposes force
        s.position_error          = s.target_norm - actual_norm;
        m.UpdateFfbSample(s);
        m.Update(QuietFrame(), 0.02);
        if (m.JustTransitionedToManual()) saw_manual_edge = true;
    }
    ...
    const auto& diag = m.GetFfbLatchDiagnostics();
    EXPECT_TRUE(diag.moving_target);           // signature (A) was blacked out...
    EXPECT_TRUE(diag.wheel_engaged_velocity);  // ...signature (B) is what fired
}

TEST(OverrideManagerTest, FfbVelocityOppositionDoesNotFalseLatchWhenWheelFollowsMovingTarget)
{
    // Same fast ramp, but the wheel obediently FOLLOWS the servo (no driver):
    // actual_norm moves the SAME direction as commanded_force_signed, just
    // lagging. Must NOT latch — the fix must not turn "servo dragging the
    // wheel through a fast ramp" into a false override.
    OverrideManager m;
    m.Configure(MakeConfigWithFfbThresholds(0.20, 0.04, 0.10, 0.30, 0.10));

    FfbInterventionSample s;
    s.active                 = true;
    s.commanded_force_signed = 0.50;
    s.commanded_force        = 0.50;

    for (int i = 0; i <= 30; ++i)
    {
        s.target_norm            = 0.05 * i;
        const double actual_norm = 0.05 * i * 0.8;   // lags, but SAME sign/direction
        s.position_error         = s.target_norm - actual_norm;
        m.UpdateFfbSample(s);
        m.Update(QuietFrame(), 0.02);
    }
...
TEST(OverrideManagerTest, FfbVelocityOppositionBelowRateGateDoesNotLatch)
{
    // Opposite sign, but |d(actual)/dt| stays under override_opposition_
    // velocity_gate (jitter-scale motion) — must not count as "opposing".
    // Guards against the new gate being too permissive.
    OverrideManager m;
    m.Configure(MakeConfigWithFfbThresholds(0.20, 0.04, 0.10, 0.30, 0.10));
    ...
    for (int i = 0; i <= 30; ++i)
    {
        s.target_norm             = 0.05 * i;         // moving; irrelevant here
        const double actual_norm  = -0.001 * i;        // opposite sign, rate -0.05/s < 0.30 gate
        s.position_error          = s.target_norm - actual_norm;
        m.UpdateFfbSample(s);
        m.Update(QuietFrame(), 0.02);
    }
...
TEST(OverrideManagerTest, FfbMultiCycleInterventionRelatchesAfterResumeDuringEnvelopeRamp)
{
    // Direct regression guard for the reported defect itself: "override ->
    // AUTO resume -> second override never fires". All four prior RESUME
    // tests (ResumeButtonEdgeReturnsToAutoBothDomains etc.) only exercise ONE
    // latch/resume cycle — that blind spot is exactly where this bug hid.
    //
    // Cycle 1: static-target latch (signature A), as in existing tests.
    // RESUME: back to AUTO.
    // Cycle 2: reproduces the actual failure mode — the AD steering envelope
    //   ramp holds moving_target=true for the whole transient — and must
    //   still latch via the new velocity signature (B).
    // RESUME again, Cycle 3: static latch once more, proving repeated
    //   cycles keep working, not just two.
    OverrideManager m;
    m.Configure(MakeConfigWithFfbThresholds(0.20, 0.04, 0.10, 0.30, 0.10));
    ...
    hi.commanded_force = 0.50;
    hi.position_error  = 0.10;
    hi.target_norm     = 0.00;   // actual = -0.10 -> wheel engaged (position)
    ...
    // Cycle 2: envelope-ramp regression scenario.
    FfbInterventionSample ramp;
    ramp.active                 = true;
    ramp.commanded_force_signed = 0.50;
    ramp.commanded_force        = 0.50;
    bool relatched = false;
    for (int i = 0; i <= 15; ++i)
    {
        ramp.target_norm         = 0.05 * i;
        const double actual_norm = 0.5 - 0.10 * i;   // opposes the servo force
        ramp.position_error      = ramp.target_norm - actual_norm;
        m.UpdateFfbSample(ramp);
        m.Update(QuietFrame(), 0.02);
        if (m.JustTransitionedToManual()) relatched = true;
    }
    EXPECT_TRUE(relatched);
    ASSERT_TRUE(m.IsLateralManual());
    ...
    // Cycle 3: static latch again.
    for (int i = 0; i < 6; ++i) { m.UpdateFfbSample(hi); m.Update(QuietFrame(), 0.02); }
    EXPECT_TRUE(m.IsLateralManual());
}

TEST(OverrideManagerTest, FfbHoldPreservesSustainDuringOscillatingRateWithStandingPositionEngagement)
{
    // Regression guard for a fragility found 2026-07-26 via real headless
    // repro measurement (team-lead "pushback" harness): under a fast,
    // sign-oscillating AD target (the envelope's own ramp), the frame-by-
    // frame sign check in signature (B) can coincidentally match the
    // servo's constant force direction on some frames even while the driver
    // holds a genuine, standing position offset the WHOLE time. Without the
    // HOLD branch, each such coincidental frame wipes ffb_sustain_accum_ to
    // 0 and the latch is badly delayed. With HOLD, standing position
    // engagement (wheel_engaged_position) pauses the clock instead of
    // resetting it on those frames, so it still reaches sustain_time
    // promptly (this test: within 10 oscillating frames, half of which
    // "coincidentally" match the force direction and half of which don't).
    OverrideManager m;
    m.Configure(MakeConfigWithFfbThresholds(0.20, 0.04, 0.10, 0.30, 0.10));

    FfbInterventionSample s;
    s.active                 = true;
    s.commanded_force_signed = -0.60;   // constant direction, saturated
    s.commanded_force        = 0.60;
    s.position_error         = 0.30;    // standing offset, held constant throughout

    double target = 0.0;
    s.target_norm = target;
    m.UpdateFfbSample(s);
    m.Update(QuietFrame(), 0.02);   // bootstrap frame

    const double rates[] = { 2.0, -2.0, 1.8, -1.8, 2.0, -2.0, 1.9, -1.9, 2.0, -2.0 };
    bool saw_manual_edge = false;
    for (double r : rates)
    {
        target += r * 0.02;   // oscillating target, stays bounded near 0
        s.target_norm = target;
        m.UpdateFfbSample(s);
        m.Update(QuietFrame(), 0.02);
        if (m.JustTransitionedToManual()) saw_manual_edge = true;
    ...
    // Sanity: confirms this scenario genuinely drives target_rate through
    // the gate on the last frame (that's the point being exercised).
    const auto& diag = m.GetFfbLatchDiagnostics();
    EXPECT_GT(std::abs(diag.target_rate), 0.30);
}
